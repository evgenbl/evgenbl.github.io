...
**Mailcommon** (библиотека почтовых приложений)

Библиотека KDE PIM, обеспечивающая поддержку почтовых приложений

---

**Mailimporter** (библиотека почтового импортера)

Библиотека почтового импортера

---

**Marble** (виртуальный глобус)

Marble - это виртуальный глобус с расширенными возможностями маршрутизации. Вы можете загружать карты различных тематик и исследовать окрестности. Запишите свое путешествие и поделитесь им с друзьями. Функции онлайн- и оффлайн-маршрутизации, GPS-слежения и записи, тематические карты (OpenStreetMap, спутниковая карта и другие), поддержка различных проекций (глобус, плоская карта, ...) и других планет, а также загрузка карт и данных о маршрутах для использования в автономном режиме

---

**Markdownpart** (markdown viewer KPart)

Этот репозиторий содержит плагин Markdown viewer, который позволяет приложениям, использующим Parts, отображать файлы в формате Markdown в целевом формате.

KParts — технология, разработанная в рамках проекта KDE с целью позволить программам использовать функциональные возможности других программ вместе с их виджетами (например, окно Konsole в Kate). Такие внедряемые приложения могут использоваться, например, для просмотра файлов PDF или ODT / MS Word прямо в окне браузера, при этом будет запущена соответствующая программа (вероятно, она предоставит также возможность изменения открытого файла)

---

**Mbox-importer** (импортирует файлы mbox в KMail)

Импортируйте файлы mbox в Gmail

---

**Messagelib** (Библиотека обмена сообщениями KDE PIM)

Библиотека сообщений KDE PIM

---

**Milou** (поисковый апплет Plasma)

Плазменный апплет, который может выполнять поиск файлов, электронных писем, контактов, событий и других данных, индексируемых KDEDesktopSearch

---

**Minute** (программное обеспечение для музыкального образования)

Minuet - это программное обеспечение KDE для музыкального образования, такого как тренировка слуха, чтение с первого взгляда, сольфа, гаммы, ритм, гармония и импровизация. Minuet широко использует возможности MIDI

---

**Modemmanager-qt** (привязки ModemManager Qt)

modemmanager-qt - это модуль KDE Frameworks 1-го уровня, реализующий интерфейс ModemManager DBus API

---

**Networkmanager-qt** (привязки Qt для NetworkManager)

Этот пакет содержит привязки Qt для NetworkManager

---

**Okteta** (шестнадцатеричный редактор для KDE)

Шестнадцатеричный редактор KDE для просмотра и редактирования исходных данных файлов

---

**Okular** (средство просмотра документов)

Okular - это унифицированное средство просмотра документов для KDE. Okular сочетает в себе отличные функциональные возможности с универсальностью поддержки различных типов документов, таких как PDF, Postscript, DjVu, CHM, XPS, ePub и других.

Homepage: https://okular.kde.org

---

**Oxygen** (тема для оконного менеджера KDE)

Тема для оконного менеджера KDE Oxygen

---

**Oxygen-fonts** (Шрифты для настольных компьютеров KDE)

Семейство шрифтов для настольных компьютеров и графического интерфейса пользователя для интегрированного использования с KDE desktop

---

**Oxygen-gtk2** (движок Oxygen GTK+)

Oxygen-Gtk2 - это порт темы виджета KDE Oxygen на GTK+. Его основная цель - обеспечить визуальную согласованность между приложениями на основе GTK+ и qt, работающими под управлением KDE. Вторая цель состоит в том, чтобы создать отдельную привлекательную тему GTK +, которая бы хорошо работала в других средах рабочего стола. В отличие от других попыток переноса темы KDE oxygen на GTK+, эта попытка не зависит от Qt.

See: http://projects.kde.org/projects/playground/artwork/oxygen-gtk

---

**Oxygen-icons 5** (тема Oxygen для KDE)

Oxygen предоставляет полную и современную тему значков для KDE

---

**Palapeli** (игра-головоломка)

Palapeli - это игра-головоломка для одного игрока. В отличие от других игр этого жанра, вы не ограничены в размещении фигур на воображаемых сетках. Фигуры могут свободно перемещаться. Кроме того, Palapeli обладает реальной сохраняемостью, то есть все, что вы делаете, сохраняется немедленно

---

**Parley** (словарный тренажер)

Parley - это словарный тренажер для KDE

---

**Partitionmanager** (графический менеджер разделов)

Простое управление дисками, разделами и файловыми системами на рабочем столе KDE. Создавайте, изменяйте размер, перемещайте, копируйте, создавайте резервные копии, восстанавливайте или удаляйте разделы

---

**Picmi** (логическая игра-головоломка)

Picmi - это логическая игра-головоломка для одного игрока. Цель состоит в том, чтобы раскрасить ячейки в соответствии с номерами, указанными сбоку от игрового поля, чтобы обнаружить скрытый узор или картинку

---

**Pim-data-exporter** (экспорт настроек KDE PIM)

Импортируйте и экспортируйте настройки KDEPIM

---

**Pim-sieve-editor** (редактор скриптов Sieve)

pim-sieveeditor - это редактор скриптов Sieve, используемый для фильтрации электронной почты на почтовом сервере

---

**Pimcommon** (общие библиотеки для KDE PIM)

pimcommon содержит общие библиотеки для KDE PIM

---

**Plasma-browser-integration** (интеграция браузеров в Plasma)

Пакет plasma-browser-integration содержит компоненты, необходимые для интеграции браузеров в рабочий стол Plasma

---

**Plasma-desktop** (KDE Plasma Desktop)

Предоставляет среду для запуска приложений, управления ими и интеграции взаимодействия приложений. Она разработана как универсальная среда для всех видов настольных приложений, а не только для приложений, созданных на платформе KDE. Он наилучшим образом интегрируется с приложениями, соответствующими стандартам, используемым платформой KDE

---

**Plasma-disks** (монитор работоспособности дисков)

Plasma Disks отслеживает данные SSD на дисках и предупреждает пользователя о появлении признаков неминуемого сбоя

---

**Plasma-firewall** (Plasma брандмауэр KCM)

Модули Plasma KConfig для настройки брандмауэра

---

**Plasma-framework** (библиотеки и компоненты Plasma)

Этот пакет содержит библиотеки Plasma и компоненты среды выполнения, такие как поддержка QML для Plasma

---

**Plasma-integration** (Плагины для интеграции с темой Plasma)

Plasma Integration - это набор плагинов, отвечающих за лучшую интеграцию приложений Qt при запуске в KDE Plasma workspace

---

**Plasma-nm** (апплет управления сетью KDE)

Этот пакет содержит апплет управления сетью KDE для Plasma

---

**Plasma-pa** (программа регулировки громкости Plasma PA - PulseAudio)

Программа Plasma для управления громкостью звука с помощью PulseAudio

---

**Plasma-sdk** (Приложения для разработки плазмы)

Приложения, полезные для разработки плазмы

---

**Plasma-systemmonitor** (монитор системных ресурсов)

plasma-systemmonitor предоставляет интерфейс для мониторинга системных датчиков, информации о процессе и других системных ресурсов. Он построен поверх системы faces, которая также используется для предоставления виджетов для plasma-desktop, и использует демон состояния системы K для предоставления информации с датчиков

---

**Plasma-vault** (создание зашифрованных хранилищ)

Апплет Plasma и сервисы для создания зашифрованных хранилищ. Plasma Vault предлагает надежные функции шифрования, представленные в удобной для пользователя форме. Блокируйте и шифруйте документы, а также скрывайте их при входе пользователя в систему. Эти "хранилища" могут быть легко расшифрованы и открыты

---

**Plasma-wayland-protocols** (Протоколы Plasma для Wayland)

Этот проект содержит xml-файлы нестандартных протоколов Wayland, используемых Plasma

---

**Plasma-workspace** (Рабочее пространство KDE Plasma)

Рабочее пространство Plasma обеспечивает поддержку виджетов KDE Plasma, интегрированный поиск, управление оборудованием и высокую степень настраиваемости

---

**Plasma-workspace-wallpapers** (Обои для рабочего пространства Plasma)

Дополнительные обои для рабочего пространства KDE Plasma

---

**Polkit-kde-agent-1** (средство проверки подлинности KDE polkit)

Этот демон предоставляет пользовательский интерфейс polkit authenticator для KDE.

See also: https://projects.kde.org/projects/kde/workspace/polkit-kde-agent-1

---

**Powerdevil** (инструмент управления питанием)

PowerDevil - это инструмент настройки для управления питанием в Plasma. С помощью этого модуля вы можете определять поведение вашего компьютера в различных ситуациях, что позволяет максимально экономить энергию

---

**Poxml** (перевод XML-файлов DocBook)

Это набор инструментов, которые облегчают перевод XML-файлов DocBook с помощью файлов сообщений Gettext (PO-файлов). Также включены несколько утилит командной строки для работы с XML-файлами DocBook, PO-файлами и файлами шаблонов PO.

DocBook — подмножество языка разметки SGML или XML, предназначенное для разметки документов, такое же, как HTML для разметки веб-документов.Официальный стандарт OASIS.
...
**Print-manager** - диспетчер печати (управление заданиями печати и принтерами)

Print Manager - Диспетчер печати - это утилита в системном трее, которая отображает текущие задания печати, предупреждения принтера и ошибки.

Homepage: https://projects.kde.org/projects/kde/kdeutils/print-manager

---

**Prison** - это API штрих-кодов, который в настоящее время предлагает Qt API для создания штрих-кодов QRCode и Datamatrix.

Homepage: http://anongit.kde.org/prison

---

**Pulseaudio-qt** (привязки PulseAudio Qt)

Pulseaudio Qt позволяет программам, использующим библиотеку Qt, регулировать громкость и реагировать на изменения в системах, использующих PulseAudio.

Homepage: https://cgit.kde.org/pulseaudio-qt.git/

---

**Purpose** - назначение (предлагает действия для определенной цели)

Этот фреймворк предоставляет возможность создавать интегрированные сервисы и действия в любом приложении без необходимости их специальной реализации. Purpose предложит им механизмы для составления списка различных альтернатив для выполнения с учетом запрашиваемого типа действия и упростит компоненты, чтобы все плагины могли получать всю необходимую им информацию

---

**Qqc2-breeze-style** (стиль QQC2 Breeze)

Это чистый стиль Qt Quick/Kirigami Qt Quick Controls. В отличие от QQC2-Desktop-Style, он не зависит от виджетов Qt и системного QStyle.

Модуль Qt Quick Controls предоставляет набор элементов управления, которые можно использовать для создания полноценных интерфейсов в Qt Quick.

Kirigami, развиваемого сообществом KDE для быстрой разработки приложений с адаптивным интерфейсом, работающим без изменения на настольных и мобильных системах. Фреймворк является надстройкой над Qt Quick Controls и использует в качестве основы уже предоставляемые в Qt Quick элементы, такие как кнопки и поля ввода.
...
**Qqc2-desktop-style** (интеграция с настольным компьютером для Qt Quick Controls 2)

QC2-Desktop-Style - это стиль для Qt Quick Controls 2 (QC2), который использует QStyle для рисования элементов управления, чтобы придать им естественный внешний вид.

Homepage: https://api.kde.org/frameworks/qqc2-desktop-style/html/

---

**Rocs** (среда разработки теории графов)

Rocs - это среда разработки теории графов для всех, кто интересуется разработкой и анализом графовых алгоритмов (например, преподавателей, студентов, исследователей). Для всех этих пользователей Rocs предоставляет простой в использовании визуальный редактор структуры данных и мощный скриптовый движок для выполнения алгоритмов. Алгоритмы задаются на JavaScript

Homepage: https://www.kde.org/applications/education/rocs

---

**Sddm** (X11 display manager, основанный на QML)

SDDM - это современный дисплейный менеджер для X11, который должен быть быстрым, простым и красивым. Он использует современные технологии, такие как QtQuick, что, в свою очередь, дает дизайнеру возможность создавать плавные анимированные пользовательские интерфейсы. SDDM чрезвычайно удобен в использовании. Есть несколько примеров тем, которые распространяются вместе с SDDM. Их можно использовать в качестве отправной точки для создания новых тем.

Homepage: https://github.com/sddm/sddm

---

**Sddm-kcm** (настройка SDDM в kde)

Модуль KCM для SDDM. Этот модуль позволяет настроить SDDM в системных настройках KDE.

Homepage: https://github.com/sddm/sddm-kcm

---

**Skanlite** (программа для сканирования)

Skanlite - это облегченное приложение для сканирования, основанное на libksane.

libKSane

Библиотека для работы со сканерами LibKSane — это интерфейс KDE для библиотеки SANE для управления плоскими сканерами.

Эта библиотека предоставляет виджет для управления сканером KSaneWidget. Внутри он будет использовать библиотеку SANE.
...
**Solid** (абстракция аппаратного обеспечения настольного компьютера)

Solid - это платформа для интеграции устройств. Она предоставляет способ запроса и взаимодействия с оборудованием независимо от базовой операционной системы. Она обеспечивает обнаружение оборудования, управление питанием и сетью.

Homepage: https://api.kde.org/frameworks/solid/html/

---

**Sonnet** (средство проверки орфографии на нескольких языках)

Sonnet - это библиотека проверки орфографии на основе плагинов для приложений на базе Qt. Она поддерживает несколько различных плагинов, включая Spell, Enchant, ASpell и HUNSPELL. Он также поддерживает автоматическое определение языка, основанное на комбинации различных алгоритмов

---

**Spectacle** (программа для создания снимков экрана)

Spectacle - это утилита для создания снимков экрана для рабочего стола KDE. Spectacle также может использоваться в настольных средах, отличных от KDE X11

---

**Step** (интерактивный физический симулятор)

Step - это интерактивный физический симулятор. Он работает следующим образом: вы размещаете на сцене несколько тел, добавляете некоторые силы, такие как гравитация или пружины, затем нажимаете "Имитировать", и Step показывает вам, как ваша сцена будет развиваться в соответствии с законами физики. С помощью Step вы сможете не только узнать, но и почувствовать, как работает физика

---

**Svgpart** (SVG-плагин для KDE)

KPart предназначен для просмотра SVG-файлов в KDE

---

**Sweeper** (удаляет следы вашей деятельности)

Sweeper помогает убрать нежелательные следы, которые пользователь оставляет в системе.

Homepage: https://projects.kde.org/projects/kde/kdeutils/sweeper

---

**Syndication** (библиотека синтаксического анализа RSS/Atom)

Syndication - это библиотека синтаксического анализа RSS / Atom, поддерживающая каналы RSS (0.9/1.0, 0.91..2.0) и Atom (0.3 и 1.0). Синдикация предлагает унифицированный, не зависящий от формата просмотр проанализированной ленты, так что используемому приложению не нужно различать форматы лент

---

**Syntax-highlighting** - подсветка синтаксиса (syntax highlighting for Kate)

Механизм подсветки синтаксиса для структурированного текста и кода

---

**Systemsettings** - системные настройки (утилита системных настроек)

Центр управления программным обеспечением и рабочими пространствами KDE

---

**Threadweaver** (помощник по многопоточному программированию)

ThreadWeaver - это помощник для многопоточного программирования. Он использует интерфейс на основе заданий для постановки задач в очередь и их эффективного выполнения. Вы просто делите рабочую нагрузку на задания, указываете зависимости между заданиями, и ThreadWeaver разработает наиболее эффективный способ распределения работы между потоками в рамках набора ограничений по ресурсам

---

**Umbrella** - это графический интерфейс для создания диаграмм на унифицированном языке моделирования (UML).
...
**Wacom tablet** (kcm для планшетов Wacom)

Этот модуль реализует графический интерфейс для драйверов Wacom Linux и расширяет его поддержкой профилей для работы с различными расположениями кнопок / перьев для каждого профиля.

Homepage: https://github.com/KDE/wacomtablet

---

**Xdg-desktop-portal-kde** (реализация для xdg-desktop-portal)

Серверная реализация для xdg-desktop-portal с использованием Qt/KF5

---

**Yakuake** (выпадающий терминал)

Yakuake - это эмулятор выпадающего терминала, основанный на KDE Konsole.

Некоторые из особенностей Yakuake включают в себя: Плавный переход от верхней части экрана к вкладкам, настраиваемый интерфейс, настраиваемые размеры и скорость анимации, сложный интерфейс D-Bus

---

**Zanshin** (Приложение для выполнения задач)

Zanshin - это мощное, но простое приложение для управления вашими повседневными действиями. Оно помогает вам организовать и снизить когнитивную нагрузку, связанную с тем, что вам приходится делать на работе и в личной жизни. Вы больше никогда ничего не забудете, благодаря тому, что ваш разум наполнен энергией. Пользовательский интерфейс Zanshin позволяет быстро просмотреть общую информацию о вашей жизни и задачах.

Homepage: https://zanshin.kde.org

---

**Zeroconf-ioslave** (Сетевой монитор для zeroconf)
